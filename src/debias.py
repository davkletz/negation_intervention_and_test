...
def get_debiasing_projection(classifier_class, cls_params: Dict, num_classifiers: int, input_dim: int,
                             ...
                             Y_dev_main=None, dropout_rate = 0, steps = False, model_name = None, layer = 1) -> np.ndarray:
    """
        ...
        if is_autoregressive:
            ...
            if steps:
                if i+1 in [1, 5, 15, 20]:
                    ...
                    dump(P, f"{abs_path}/neg_proj_matrix_{model_name}_{i+1}_{layer}")
                    dump(Ws, f"{abs_path}/neg_log_reg_hyperplans_{model_name}_{i+1}_{layer}")

            '''abs_path = "/home/dkletz/tmp/pycharm_project_99/2022-23/Interventions/test_with_negation/outputs"

            dump(P, f"{abs_path}/neg_proj_matrix_{model_name}_{i}_{layer}")
            dump(Ws, f"{abs_path}/neg_log_reg_hyperplans_{model_name}_{i}_{layer}")'''
